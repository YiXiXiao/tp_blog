    ...
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        ...
        if ($info !== false) {
            ...
            $srcImg = $createFun($image);
                   //补宽
        if($maxWidth-$width > 0){
            $height = $height +(($height/$width) * ($maxWidth-$width));
            
            $width = $maxWidth;
        }

         if($maxHeight-$height > 0){
            $width = $width + (($width/$height) * ($maxHeight-$height));
            ...
            if('png'==$type){
             ...
             //$white = imagecolorallocate($thumbImg,255,255,255);
              ...
              $posx =(int) ($maxWidth - $width)/2;
              $posy = (int) ($maxHeight - $height)/2;
            // 复制图片
            if (function_exists("ImageCopyResampled"))
                imagecopyresampled($thumbImg, $srcImg, $posx, $posy, 0, 0, $width, $height, $srcWidth, $srcHeight);
            else
                imagecopyresized($thumbImg, $srcImg, $posx, $posy, 0, 0, $width, $height, $srcWidth, $srcHeight);
